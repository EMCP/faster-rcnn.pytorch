...
class AffineGridGenV2(Module):
    ...
    def forward(self, input1):
        ...
        output = torch.bmm(self.batchgrid.view(-1, self.height * self.width, 3), torch.transpose(input1, 1, 2)).view(-1,
                                                                                                                     self.height,
                                                                                                                     self.width,
                                                                                                                     2)
...
class DenseAffine3DGridGen(Module):
    ...
    def forward(self, input1):
        ...
        # phi = torch.atan(y/x)
        phi = torch.atan(y / (x + 1e-5)) + np.pi * x.lt(0).type(torch.FloatTensor) * (
                    y.ge(0).type(torch.FloatTensor) - y.lt(0).type(torch.FloatTensor))
        phi = phi / np.pi
...
class DenseAffine3DGridGen_rotate(Module):
    ...
    def forward(self, input1, input2):
        ...
        output1 = torch.atan(
            torch.tan(np.pi / 2.0 * (output[:, :, :, 1:2] + self.batchgrid[:, :, :, 2:] * input_u[:, :, :, :]))) / (
                              np.pi / 2)
...
class Depth3DGridGen_with_mask(Module):
    ...
    def forward(self, depth, trans0, trans1, rotate):
        ...
        if depth.is_cuda:
            phi = torch.atan(y / (x + 1e-5)) + np.pi * x.lt(0).type(torch.cuda.FloatTensor) * (
                        y.ge(0).type(torch.cuda.FloatTensor) - y.lt(0).type(torch.cuda.FloatTensor))
        else:
            phi = torch.atan(y / (x + 1e-5)) + np.pi * x.lt(0).type(torch.FloatTensor) * (
                        y.ge(0).type(torch.FloatTensor) - y.lt(0).type(torch.FloatTensor))
